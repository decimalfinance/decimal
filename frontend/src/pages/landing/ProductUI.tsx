...
export function PaymentsTable({
  ...
  return (
    <div className="lp-pmt-table">
      <div className="lp-pmt-header">
        <div>Recipient</div>
        <div>Destination</div>
      ...
      {rows.map((r) => (
        <div key={r.rcp} className="lp-pmt-row">
          <div>
            <div style={{ fontSize: 12, color: 'var(--ax-text)' }}>{r.rcp}</div>
            <div className="mono" style={{ fontSize: 10, color: 'var(--ax-text-muted)' }}>
              {r.inv}
            </div>
          </div>
          <div className="mono" style={{ fontSize: 12, color: 'var(--ax-text-secondary)' }}>
            {r.dst}
          </div>
          ...
          <div>
            <span className={`lp-pmt-status lp-pmt-${statusTone}`}>
              <span className="lp-pmt-dot" />
              {status}
            </span>
          </div>
          <div
            className="mono"
            style={{ fontSize: 11, color: showSig ? 'var(--ax-accent)' : 'var(--ax-text-faint)' }}
          >
            {showSig ? r.sig : '—'}
          </div>
        </div>
