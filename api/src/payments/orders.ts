...
export async function ensurePaymentOrderAuditRequest(
  args: PaymentActorInput & {
    organizationId: string;
    paymentOrderId: string;
  },
...
function validateCounterpartyWalletForPaymentOrder(wallet: Pick<CounterpartyWallet, 'label' | 'trustState' | 'isActive'>) {
  ...
  if (wallet.trustState !== 'trusted') {
    throw new Error(
      `Counterparty wallet "${wallet.label}" is ${wallet.trustState ?? 'unreviewed'} — review and mark it as trusted before routing a payment to it.`,
    );
  }
}
