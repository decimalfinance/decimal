...
- it exists at `confirmed`, the commitment required for the `settled` assertion. Below that
  the transaction reads as not-yet-available, the order stays non-terminal, and the
  reconciler retries.
